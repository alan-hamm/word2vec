...
%macro update_name_list(T=1);
	%if %eval(&t=1) %then %do;
		...
		data harvard_firstnames;
			...
			if lengthn(firstname) > 4;
			/* temporarily remove name Said  as F7 notes use verb Said with no */
			/* seeming(visual check and cleaning) apparent use of name Said    */
			if lowcase(firstname) ^= 'said';
		run;
...
%macro string_shredder(aaa=1);
	...
	* defensive programming;
	...
	%if %eval(&aaa=1) %then %do;
		libname a "\\cdc.gov\csp_Project\CIPSEA_DHIS_NHIS\Production\NLP_DEV\development_data";
		proc copy in=work out=a;
			select f7notes_2 f7_shredded_2 train_data_2;
		run;
		libname a clear;
	%end;
%mend; %string_shredder()

filename a "\\cdc.gov\csp_Project\CIPSEA_DHIS_NHIS\Production\GitRepository\Code\NLP\train_model-8.py";
data _null_;
	set train_data_2 end=last;
	file a;
